...
const storage = multer.diskStorage({
    destination: function(req, file, cb) {
        cb(null, './public/img/product') // ตำแหน่งจัดเก็บไฟล์
    },
...
router.get('/cake', (req, res) => {
    Product.find().exec((err, doc) => {
        res.render('cake', { products: doc })
    })
})
router.get('/cupcake', (req, res) => {
    Product.find().exec((err, doc) => {
        res.render('cupcake', { products: doc })
    })
})
router.get('/topping', (req, res) => {
    Product.find().exec((err, doc) => {
        res.render('topping', { products: doc })
    })
})
...
router.post('/login', (req, res) => {
    ...
    const timeExpire = 900000 // 900 วินาที
